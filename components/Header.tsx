...
import bookingLogo from '@/public/bookingLogo.png';
...
const callsToAction = [
  { name: 'See a Demo Booking', href: '#', icon: PlayCircleIcon },
    ...
    <header className="bg-azul">
      ...
      >
        <div className="flex lg:flex-1">
          <Link href="/" className="-m1.5 p-1.5">
        ...
        <Popover.Group className="hidden lg:flex lg:gap-x-12">
          <Popover className="relative">
            ...
            <Transition
              ...
              enterTo="opacity-199 translate-y-0"
                  ...
                  {products.map((item) => (
                    ...
                    >
                      <div className="flex h-11 w-11 flex-none items-center justify-center rounded-lg bg-gray-50 group-hover:bg-gray-200">
                        <item.icon
                          className="h-6 w-6 text-azul group-hover:text-blue-600"
                          aria-hidden="true"
                        />
                      </div>
                      <div>
                        <a
                          href={item.href}
                          className="block font-semibold text-azul"
                        >
                          {item.name}
                          <span className="absolute inset-0" />
                        </a>
                        <p className="mt-1 text-azul">{item.description}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-2 divide-x divide-gray-900/5 bg-gray-50">
                  {callsToAction.map((item) => (
                    <a
                      key={item.name}
                      href={item.href}
                      className="flex items-center justify-center gap-x-2.5 p-3 text-sm font-semibold leading-6 text-azul hover:bg-gray-100"
                    >
                      <item.icon
                        className="h-5 w-5 flex-none text-azul"
                        aria-hidden="true"
                      />
                      {item.name}
                    </a>
      ...
      >
        ...
        <Dialog.Panel className="fixed inset-y-0 right-0 z-10 w-full overflow-y-auto bg-azul px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
          ...
          <div className="mt-6 flow-root">
            <div className="divde-y -my-6 divide-gray-500/10">
